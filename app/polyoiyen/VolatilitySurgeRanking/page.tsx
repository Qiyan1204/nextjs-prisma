...
export default function VolatilitySurgeRankingPage() {
  const router = useRouter();
  const [rows, setRows] = useState<SurgeRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [updatedAt, setUpdatedAt] = useState<string>("");
  const [selectedEventId, setSelectedEventId] = useState<string>("");
  const [contextMenu, setContextMenu] = useState<{ x: number; y: number; row: SurgeRow } | null>(null);
    ...
    try {
      ...
      const mapped = await Promise.all(
        candidates.map(async (event) => {
          ...
          const yesRating = data?.metrics?.yes?.totalVolatilityRating ?? 0;
          const noRating = data?.metrics?.no?.totalVolatilityRating ?? 0;
          const surgeScore = yesRating + noRating;
          const avgPerHour =
            ((data?.metrics?.yes?.averageVolatilityRatingPerHour ?? 0) +
              (data?.metrics?.no?.averageVolatilityRatingPerHour ?? 0));
      ...
      const ranked = mapped
        .filter((x): x is SurgeRow => Boolean(x))
        .sort((a, b) => {
      ...
      setRows([]);
      setError(e instanceof Error ? e.message : "Failed to compute volatility surge ranking");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  function renderInlineChart(row: SurgeRow) {
    const chartW = 920;
    const chartH = 250;
    const padL = 46;
    const padR = 16;
    ...
    return (
      <div style={{ borderBottom: "1px solid rgba(255,255,255,0.05)", padding: "12px 12px 14px" }}>
        ...
        <div style={{ display: "flex", gap: 12, marginTop: 8, fontSize: 11 }}>
          <span style={{ color: "#34d399" }}>■ YES step</span>
          <span style={{ color: "#f87171" }}>■ NO step</span>
        </div>
        ...
        <div style={{
          ...
          {hoveredPoint && hoveredPoint.eventId === row.eventId
            ...
            : "Tooltip: hover any chart point to inspect YES/NO step values."}
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ background: "#160c03", minHeight: "100vh", color: "white", fontFamily: "'DM Sans', sans-serif" }}>
      <PolyHeader active="EliteVolatilitySurge" />
      ...
      <main style={{ maxWidth: 1120, margin: "0 auto", padding: "28px 24px 64px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginTop: 16, flexWrap: "wrap" }}>
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.55)" }}>
            Window: last 1 hour · Updated: {updatedAt || "--:--:--"}
          </div>
          <button
        ...
        {top && (
          <div style={{
            ...
            <div style={{ marginTop: 5, fontSize: 12, color: "rgba(255,255,255,0.62)" }}>
              Surge Score: <span style={{ color: "#f97316", fontWeight: 800 }}>{top.surgeScore.toFixed(2)}</span>
              {" · "}
              YES: <span style={{ color: "#34d399", fontWeight: 700 }}>{top.yesRating.toFixed(2)}</span>
              {" · "}
              NO: <span style={{ color: "#f87171", fontWeight: 700 }}>{top.noRating.toFixed(2)}</span>
            </div>
          </div>
        )}

        <div style={{ position: "relative", marginTop: 14, border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, overflow: "hidden", background: "rgba(255,255,255,0.02)" }}>
          <div style={{
            display: "grid",
            gridTemplateColumns: "70px minmax(260px, 1fr) 120px 120px 120px 120px",
            gap: 8,
            padding: "10px 12px",
            ...
            <span>Rank</span>
            <span>Market</span>
            <span style={{ textAlign: "right" }}>Surge</span>
          ...
          {loading && (
            <div style={{ padding: "16px 12px", color: "rgba(255,255,255,0.62)", fontSize: 13 }}>Loading volatility surge ranking...</div>
          )}
          ...
          {!loading && !error && rows.length === 0 && (
            <div style={{ padding: "16px 12px", color: "rgba(255,255,255,0.55)", fontSize: 13 }}>No markets have enough data for 1-hour surge ranking yet.</div>
          )}
          ...
          {!loading && !error && rows.map((row, index) => (
            <div key={row.eventId}>
              ...
              >
                ...
                <span style={{ textAlign: "right", color: "#f97316", fontWeight: 800 }}>{row.surgeScore.toFixed(2)}</span>
                <span style={{ textAlign: "right", color: "#34d399", fontWeight: 700 }}>{row.yesRating.toFixed(2)}</span>
                <span style={{ textAlign: "right", color: "#f87171", fontWeight: 700 }}>{row.noRating.toFixed(2)}</span>
        ...
        {!selectedRow && !loading && !error && rows.length > 0 && (
          <div style={{ marginTop: 10, fontSize: 12, color: "rgba(255,255,255,0.6)" }}>
            Tip: click any rank row to open its 1H (5-minute points) chart inline.
          </div>
        )}
      </main>
    </div>
